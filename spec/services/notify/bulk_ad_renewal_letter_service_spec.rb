...
RSpec.describe Notify::BulkAdRenewalLettersService do
  describe ".run" do
    ...
    it "sends the relevant registrations to the Notify::AdRenewalLetterService" do
      ...
      expect(Notify::AdRenewalLetterService).to_not receive(:run).with(registration: non_ad_registration)
      expect(Notify::AdRenewalLetterService).to_not receive(:run).with(registration: non_matching_date_registration)
      expect(Notify::AdRenewalLetterService).to_not receive(:run).with(registration: inactive_registration)
    ...
    context "when an error happens" do
      it "notifies Airbrake without failing the whole job" do
        expect(Notify::AdRenewalLetterService).to receive(:run).with(registration: ad_registrations[0]).and_raise("An error")
