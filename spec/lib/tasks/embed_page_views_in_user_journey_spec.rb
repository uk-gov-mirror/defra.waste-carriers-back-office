...
RSpec.describe "one_off:embed_page_views_in_user_journey", type: :rake do
  ...
  let(:time_a_1) { 9.days.ago.to_datetime }
  let(:time_a_2) { 8.days.ago.to_datetime }
  let(:time_b_1) { 5.days.ago.to_datetime }
  let(:time_b_2) { 4.days.ago.to_datetime }
  ...
  before do
    ...
    collection.insert_one(user_journey_id: journey_a.id, time: time_a_1, route: "DIGITAL", page: "start_form")
    collection.insert_one(user_journey_id: journey_a.id, time: time_a_2, route: "DIGITAL", page: "location_form")
    collection.insert_one(user_journey_id: journey_b.id, time: time_b_1, route: "DIGITAL", page: "x_form")
    collection.insert_one(user_journey_id: journey_b.id, time: time_b_2, route: "ASSISTED_DIGITAL", page: "y_form")
  ...
  it "copies the time attribute correctly" do
    ...
    expect(journey_a.reload.page_views.pluck(:time).map(&:to_i)).to eq [time_a_1.to_i, time_a_2.to_i]
    expect(journey_b.reload.page_views.pluck(:time).map(&:to_i)).to eq [time_b_1.to_i, time_b_2.to_i]
  end
