      ...
      context "when the registration is upper tier" do
        ...
        context "when the registration has not been revoked or refused" do
          ...
          context "when there is an expires_on" do
            before { registration.expires_on = Time.current + 1.day }

            context "when the registration is in the standard grace window" do
              before { expect_any_instance_of(ExpiryCheckService).to receive(:in_expiry_grace_window?).and_return(true) }
            ...
            context "when the registration is not in the standard grace window" do
              before { expect_any_instance_of(ExpiryCheckService).to receive(:in_expiry_grace_window?).and_return(false) }
