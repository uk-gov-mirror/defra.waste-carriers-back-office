...
module Reports
  RSpec.describe MonthlyFinanceReportSerializer do
    describe "#to_csv" do

      let(:finance_stats) { FinanceStatsService.new(:mmyyyy).run }
      ...
      it "includes all expected columns only" do
        expect(subject).to include(
          %w[
            ...
            pay_worldpay_cnt
            pay_worldpay_tot
            pay_worldpaymissed_cnt
            pay_worldpaymissed_tot
            pay_cheque_cnt
            pay_cheque_tot
            pay_banktransfer_cnt
            pay_banktransfer_tot
            pay_writeoffsmall_cnt
            pay_writeoffsmall_tot
            pay_writeofflarge_cnt
            pay_writeofflarge_tot
            chg_cnt
            chg_bal
            chg_chargeadjust_cnt
            chg_chargeadjust_tot
            chg_copycards_cnt
            chg_copycards_tot
            chg_new_cnt
            chg_new_tot
      ...
      it "includes the correct reporting period format" do
        expect(subject).to include registration_report_period_mmyyyy
        expect(subject).not_to include registration_report_period_ddmmyyyy
      end
    end
  end
end
